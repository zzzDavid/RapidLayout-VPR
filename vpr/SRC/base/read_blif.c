...
static void add_subckt(int doall, t_model *user_models) {
	...
	while (iparse < MAX_ATOM_PARSE) {
		...
		if (ptr == NULL && toggle == 0)
			...
			toggle = 1;
			printf("-------- reading: subckt_signal_name[subckt_index_signals]: ");
			printf(subckt_signal_name[subckt_index_signals]);
			printf("\n");
		} else if (toggle == 1) {
	...
	if (doall) {
		...
		// debug
			printf("INSPECTION!\n");
			for(int i=0; i < subckt_index_signals; i++)
			{
				printf("prefetch-inspection: --- ");
				printf(subckt_signal_name[i]);
				printf("\n");
			}
		// port = cur_model->inputs;	
		// while (port) {
		// 	port = port->next;
		// 	printf("---cur_model->inputs port name is: %s\n", port->name);
		// }
		// port=NULL;
		...
		for (i = 0; i < subckt_index_signals; i++) {
			...
			port = cur_model->inputs;
			while (port) {
				printf("current port name: port->name=");
				printf(port->name);
				printf("\n");
